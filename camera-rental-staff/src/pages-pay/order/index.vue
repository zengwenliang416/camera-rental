      ...
      <view class="p-24rpx">
        ...
        >
          <view class="mb-16rpx flex items-start justify-between gap-16rpx">
            <view class="min-w-0 flex-1">
              <view class="truncate text-32rpx text-[#333] font-semibold">
                {{ item.subject || item.merchantOrderId || `支付订单 #${item.id}` }}
              </view>
              <view v-if="item.no" class="mt-6rpx truncate text-24rpx text-[#999]">
                支付单号：{{ item.no }}
              </view>
            </view>
            <dict-tag v-if="item.status != null" :type="DICT_TYPE.PAY_ORDER_STATUS" :value="item.status" />
          </view>
          ...
          <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
            ...
            <text class="min-w-0 flex-1 truncate">{{ item.merchantOrderId || '-' }}</text>
          </view>
          <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getPayOrderPage({ ...queryParams.value, pageNo, pageSize })
...
/** 查看详情 */
function handleDetail(item: PayOrder) {
  uni.navigateTo({ url: `/pages-pay/order/detail/index?id=${item.id}` })
}
</script>
