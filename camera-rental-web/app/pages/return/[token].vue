...
const api = useReturnRegistration(token)
...
onMounted(async () => {
  ...
  try {
    ...
    if (saved && context.value.status === 'DRAFT') {
      Object.assign(draft, JSON.parse(saved))
    }
  } catch (cause) {
    error.value = cause instanceof Error ? cause.message : '链接不可用'
  } finally {
    loading.value = false
  }
})
