...
- 闲鱼订单号和收件手机号后四位均可留空；机器编码承担默认订单定位职责，填写
  前两项时仅作为附加一致性校验或消歧信息。
- 机器编码和快递单号必填；机器编码统一为“型号前缀-规范编号”，编号范围为
  `01-999`，例如 `P4-01`、`P4-100`。
  输入支持小写、空格、全角/长连字符，服务端统一规范为大写 ASCII 连字符格式。
- 退回入口必须接受以下现行业务型号前缀与 `01-999` 规范编号组合：
...
- 既有设备按租户和型号稳定迁移为两位短码，旧设备编号保留为只读兼容别名；
  管理端手工新增设备时由管理员输入 `1-999` 编号，服务端将 `1-9` 规范化为
  `01-09` 并与型号前缀拼接；ERP 批量入库继续自动生成 `01` 至 `999` 的短码。
...
- 新增设备时管理员必须输入 `1-999` 编号；服务端校验大类与型号组合，从权威型号
  目录读取编号前缀，将一位编号补零后拼接最终设备编号，并拒绝租户内重复
  编号。手工创建不得递增型号 `next_sequence`；ERP 批量入库仍可使用事务编号
  预留。前端预览不构成最终设备编号权威。
...
- 租赁设备管理页通过 `GET /admin-api/rental/device/catalog` 获取权威大类和型号
  ...
  提交大类、型号和管理员输入的编号；后端读取型号前缀并拼接最终设备编号。
- 具有 `rental:device:update` 权限的管理员可通过
  `PUT /admin-api/rental/device/update` 修改设备序列号、仓库、采购金额和启用
  状态；设备编号、大类、型号和生命周期状态只能由创建、出库、回仓等权威流程
  维护，普通编辑不得修改。
- 序列号在同租户内保持唯一；空序列号按 `NULL` 保存。存在活动分配或处于非
  `AVAILABLE` 状态的设备不得通过普通编辑停用。
- 具有 `rental:device:delete` 权限的管理员可通过
  `DELETE /admin-api/rental/device/delete?id=...` 删除无业务历史的误建设备。
  删除必须在事务中锁定设备行，只允许 `AVAILABLE` 且无采购来源、分配、排期、
  当前活动设备锁、独立发货记录、物流设备关系或客户退回登记设备关系的设备，
  并使用项目逻辑删除机制。已释放或已到期的设备锁不阻止删除。
- 管理端设备列表按权限展示编辑和删除按钮；删除前必须二次确认，后端阻断原因
  使用统一业务错误展示，前端不得自行判断设备是否可删除。
