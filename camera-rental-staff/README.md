...
版本统一定义在 `src/config/staffVersion.ts`，由 manifest 和更新检查共同使用。
Android 启动、回前台或回到首页/我的/设置/登录页时，延迟检查下载目录中的 `release.json`。
请求并发去重，自动检查间隔 30 分钟；发货等作业页面不弹窗，离线不打断操作。
同一版本选择稍后提醒后保留 24 小时提醒间隔，新版本不受旧版本的间隔限制；设置中可手动检查。
仅接受预期包名、版本及固定 HTTPS 版本化 APK 地址，用户点“去升级”后打开系统浏览器下载安装。
不支持静默安装；没有提醒功能的旧版本需手动覆盖安装一次新版。

验证：`node scripts/test-staff-update.mjs`、`node scripts/test-device-quantity.mjs`。
更新元数据必须在新 APK 上传并核对 SHA-256、包名、版本、签名后发布，下载二维码无需更换。


## 员工订单与每日作业

订单页下单日期默认“全部日期”，支持今天、昨天和自定义闭区间日期；只有应用筛选后才发出范围参数。
租赁订单和未转换渠道记录共用日期参数，翻页和回到页面刷新时保留已应用条件。
下单日期按上海业务时区解释，渠道下单时间优先于内部补建时间。

每日作业从 `/rental/staff/tasks` 读取真实租户数据：今日应发、逾期未发、全部到期未发分开查看。
已知发货日的未转换渠道单也进入相应队列，只提供资料核对入口；缺失日期的记录留在全部订单查询中，
不猜测发货日期。回仓为占用结束前一日的计划，含运输、检测；实际收货仍需扫码登记。
超期、检测和维修来源于真实分配、收货、检测及设备状态，接口失败展示错误，不填充示例卡片。

“我的”直接显示安装版本和构建号、检查更新、版本变更说明；新版本说明首次未读时显示提示。
App 以 `plus.runtime` 安装版本为准，H5/小程序回退到构建版本；随包说明位于 `src/config/staffReleaseNotes.ts`。

验证：`node scripts/test-order-search.mjs`、`node scripts/test-daily-tasks.mjs`、`node scripts/test-mobile-workbench.mjs`。
页面验收用的本地 fixture 服务独立于源码和打包输入，不接入生产 API，也不打入 APK。
