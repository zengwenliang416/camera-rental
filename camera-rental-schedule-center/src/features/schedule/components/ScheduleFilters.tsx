...
export function ScheduleFilters({
  ...
  return (
    <section className="sc-filter-panel grid gap-3 rounded-2xl p-3 lg:grid-cols-[minmax(160px,0.8fr)_minmax(220px,1.25fr)_minmax(150px,0.72fr)_minmax(170px,0.82fr)_minmax(140px,0.64fr)_auto]">
      <label className="grid gap-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--sc-ink-muted)]">
        {labels.model}
        <select
          value={selectedModelId}
          onChange={(event) => onModelChange(event.target.value)}
          className="sc-form-control min-h-11 rounded-xl border px-3 text-xs font-bold text-[var(--sc-ink)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--sc-focus)]"
        >
      ...
      <label className="grid gap-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--sc-ink-muted)]">
        ...
        <span className="relative">
          ...
          <input
            ...
            className="sc-form-control min-h-11 w-full rounded-xl border pl-10 pr-3 text-xs font-semibold text-[var(--sc-ink)] placeholder:text-[var(--sc-ink-muted)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--sc-focus)]"
          />
        </span>
      </label>
      ...
      <label className="grid gap-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--sc-ink-muted)]">
        ...
        <select
          value="14"
          disabled
          className="sc-form-control min-h-11 rounded-xl border px-3 text-xs font-bold text-[var(--sc-ink)] disabled:cursor-not-allowed disabled:opacity-80"
        >
      ...
      <div className="flex items-end">
        <div className="sc-segmented grid min-h-11 w-full grid-cols-2 rounded-xl p-1 lg:w-auto">
          {[
