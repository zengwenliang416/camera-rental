...
page or menu. Creating a device no longer accepts a client-selected device
number; the backend locks the selected model row and allocates the next
two-digit number.
...
- Added transactional `01-99` device-number reservation through the model
  mapper's row-lock query.
- Updated manual device creation and configured ERP inbound models to use the
  catalog allocator. Unknown ERP models retain the existing legacy numbering
  path and remain uncategorized.
- Added category/model quick-create dialogs in the existing device-create
  dialog, catalog refresh and automatic selection of the saved record.
...
- Existing `TenantBaseDO`, MyBatis Plus mappers and `selectOneForUpdate`.
...
- `/Users/wenliang_zeng/workspace/tool/apache-maven-3.9.10/bin/mvn -o -pl yudao-module-rental/yudao-module-rental-biz -am -Dtest=RentalDeviceCatalogServiceTest,RentalDeviceAdminServiceTest,RentalDeviceInboundServiceTest,RentalDeviceInboundCategoryTest,RentalDeviceCodeStandTest -Dsurefire.failIfNoSpecifiedTests=false test`
...
- Each model intentionally supports at most 99 two-digit device numbers. A
  model whose `next_sequence` exceeds 99 requires a reviewed numbering-policy
