<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['promotion:banner:create'])"
...
import { getPromotionBannerPage } from '@/api/mall/promotion/banner'
...
const list = ref<PromotionBanner[]>([]) // 列表数据
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getPromotionBannerPage({ ...queryParams.value, pageNo, pageSize })
...
/** 新增 Banner */
function handleAdd() {
  uni.navigateTo({ url: '/pages-mall/promotion/banner/form/index' })
}

/** 查看详情 */
function handleDetail(item: PromotionBanner) {
  uni.navigateTo({ url: `/pages-mall/promotion/banner/detail/index?id=${item.id}` })
}

/** 初始化 */
onMounted(() => {
  uni.$on('mall:promotion-banner:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('mall:promotion-banner:reload', reload)
})
</script>
