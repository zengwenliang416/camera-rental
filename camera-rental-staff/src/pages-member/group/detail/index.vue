  ...
  <view class="yd-page-container">
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="hasAccessByCodes(['member:group:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="hasAccessByCodes(['member:group:delete'])"
...
import { deleteMemberGroup, getMemberGroup } from '@/api/member/group'
...
const formData = ref<MemberGroup>() // 详情数据
...
function handleEdit() {
  uni.navigateTo({
    url: `/pages-member/group/form/index?id=${props.id}`,
  })
}
