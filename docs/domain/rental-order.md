# 租赁订单领域

## 核心对象

| 对象 | 含义 |
|---|---|
| 商品 SKU | 可被客户选择的商品类型，例如“索尼 A7M4” |
| 设备实例 | 一台可追踪的具体设备，例如 `A7M4-0001` |
| 租赁订单 | 客户、渠道、租期和金额组成的业务订单 |
| 租赁明细 | 订单中的商品、数量、租期和计价信息 |
| 设备分配 | 将具体设备实例分配给租赁明细 |
| 渠道订单 | 闲鱼、Web、小程序等外部订单的映射 |

SKU 是商品销售和展示单位，设备实例是排期、出库、归还和维修单位。二者不能混为一个库存字段。

## 建议表

```text
rental_product_config
rental_device
rental_package
rental_package_item
rental_order
rental_order_item
rental_schedule
rental_device_assignment
rental_delivery
rental_inspection
rental_maintenance
rental_deposit
rental_channel_order
```

最终表结构必须遵循后端现有逻辑删除、租户字段、审计字段和数据库命名规范。新增表必须提供增量 SQL，不能修改已执行的历史迁移。

## 订单生命周期

订单状态应以明确的状态机实现，至少覆盖：

```text
待确认 -> 待支付（SKU 容量临时预留） -> 已支付（预留确认）
       -> 待分配 -> 已分配
       -> 待出库 -> 已出库 -> 租赁中 -> 待归还
       -> 待检测 -> 已完成
```

异常分支包括取消、支付失败、库存冲突、换机、提前归还、退款、损坏赔偿和人工复核。具体状态值在实现前应结合现有项目字典和支付模块确认。

渠道订单创建时按 SKU、数量和设备占用半开区间预留容量，不立即绑定具体设备。
线下手动录单要求运营人员在填写租期后直接选择具体设备实例；创建订单、设备分配
和排期写入在同一事务中完成。渠道订单仍可由运营人员在拣货前选择或由员工扫码
确认，并在同一事务中完成实例冲突复查、设备分配和排期写入。

## 闲鱼购买数量与实际设备台数

- `xianyu_order.goods_quantity` 保留渠道购买数量，可能用于凑金额，不能作为实际设备台数。
- 新转换的闲鱼租赁明细 `rental_order_item.quantity` 默认 1 台；实付租金与渠道原始数据不变。
- 确实需要多台时，在手机订单详情、发货页或管理后台订单详情修改实际台数（1–999）。
- `PUT /admin-api/rental/order-item/{id}/quantity` 接收 `quantity` 和 `expectedQuantity`，
  需 `rental:device:assign` 或 `rental:xianyu:ship` 权限。后端核对租户、订单关联、待分配状态、
  渠道待发货状态及未结算条件；存在任意未删除分配历史或发货记录时拒绝修改。
- 写入在事务中按渠道订单、租赁订单、明细的顺序加锁。当前数量不等于 `expectedQuantity`
  时返回冲突，调用方刷新后重新核对。后续同步保留人工设置的台数。
- 明细 `quantity_source` 区分 LEGACY（历史未确认）、DEFAULT（默认单台）和 CONFIRMED（人工确认）；
  保存相同台数也记录 CONFIRMED，后续同步保留该来源。员工订单详情显示历史未确认提示。
- 旧数据不能仅根据渠道购买数量批量改为 1。迁移 `20260924_062_device_quantity_provenance.sql`
  只修正单明细、系统创建且从未修改、待分配且渠道未发货、无分配/发货历史的计价数量副本，
  记录修改前后数量和原因到 `rental_device_quantity_correction`；保留租金和渠道购买数量。
  人工修改、多明细、已有履约事实或来源不确定的订单保持原值，逐单核对。
- 旧单设备发货参数要求只有一条明细且实际数量为 1；新增 `deviceIds` 支持同订单多设备同运单发货，必须扫齐整单实际数量（最多 100 台），不能只扫一台完成多台订单。

## 金额规则

- 后端使用整数分保存金额。
- Java、JavaScript 和数据库结算不得使用浮点数。
- 闲鱼订单的 `pay_amount` 按当前接口文档和项目业务口径作为实付金额来源。
- 退款金额、押金退还和设备赔偿分别建模，不默认互相抵扣。
- 未能解析租期的订单仍计入收款统计，但不能伪装成已完成按服务日分摊。
- 每日分摊金额之和必须严格等于订单实付金额，分币余数放到最后一个计租日。

## 渠道幂等

渠道订单必须保存：

- 渠道标识
- 应用或店铺授权标识
- 外部订单号
- 外部状态
- 原始响应
- 标准化字段
- 最近同步时间
- 转换版本和错误信息

`channel + application_id + external_order_no` 应形成唯一业务约束。同步、推送重放和详情补拉不得重复创建租赁订单。

## 敏感信息

订单详情和日志应遵守最小披露原则。手机号、地址、身份证、支付凭证、AppSecret 和完整签名不得出现在普通看板、日志、测试快照或前端返回中。

线下录单可调用 `POST /admin-api/rental/address/parse`（权限 `rental:order:create`）临时识别
“姓名 + 手机号 + 地址”文本。后端在租户快递100配置开启且存在 `API Key + API Secret`
时调用官方智能地址解析；供应商关闭、凭据缺失或调用失败时退化为本地确定性规则。原始文本
不持久化并关闭接口请求体访问日志；响应只用于填入表单，已有字段冲突必须人工确认，订单仍需
操作员核对后单独提交。
## 订单来源

`rental_order.source_type` 区分来源：`XIANYU` 由闲鱼渠道订单转换产生，
`OFFLINE` 由管理端手动录单（`POST /admin-api/rental/order/create-manual`，权限
`rental:order:create` 与 `rental:device:assign`）产生。线下订单创建即
`PENDING_ALLOCATION` + `READY`；请求明细携带具体 `deviceIds`，每台设备立即创建
对应的分配和占用排期，
订单号为 `OFF-` + 19 位零填充自增 id；客户主档存于 `rental_customer`（手机号
AES 加密、完整号码等值反查），配送信息存于 `rental_order_delivery`
（EXPRESS/ERRAND/SELF_DELIVERY，收货人手机号与地址加密）。跑腿/自送订单在分配
入口选好设备时即同事务完成出库（assignment 推进 DISPATCHED、设备推进 RENTED，
复用设备出库写路径）；`POST /admin-api/rental/order/confirm-outbound` 保留为兜底
入口，仅推进仍为 ASSIGNED 的分配且幂等。两条路径都不产生 `rental_delivery` /
`rental_device_shipment` 记录；快递配送的线下订单走线下快递流程。报表按
`source_type` 实际值统计来源。

### 员工端订单搜索范围

`/rental/order/staff-page` 查询已生成内部租赁订单，返回准备状态与阻塞原因。
`/rental/order/staff-channel-page` 使用相同员工查询权限，只读查询尚未关联有效内部订单的
渠道记录，按当前租户隔离并独立分页。手机“全部”同时展示两个区域；渠道记录不能当作
内部订单进入分配/发货，不从渠道购买数量推断实际设备台数。查询不会触发重评或建单。
姓名、手机号、订单号等为任一字段包含匹配，不对输错数字做近似匹配；无结果与接口失败
分别提示。已同步并不等于已具备履约条件，备注更正仍经现有解析与重评服务处理。
