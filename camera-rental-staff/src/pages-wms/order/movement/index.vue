<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['wms:movement-order:create'])"
...
import { getMovementOrderPage } from '@/api/wms/order/movement'
...
const list = ref<MovementOrder[]>([]) // 列表数据
...
function handleAdd() {
  uni.navigateTo({
    url: '/pages-wms/order/movement/form/index',
  })
}

/** 查看详情 */
function handleDetail(item: MovementOrder) {
  uni.navigateTo({
    url: `/pages-wms/order/movement/detail/index?id=${item.id}`,
  })
}

/** 初始化 */
onMounted(() => {
  uni.$on('wms:movement-order:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('wms:movement-order:reload', reload)
})
</script>
