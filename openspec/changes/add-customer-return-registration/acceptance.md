...
- 新增型号时可配置独立编号前缀；新增设备选择该型号后，设备编号由后端按前缀和
  `01-99` 序号自动生成。伪造大类与型号组合或跨租户目录 ID 会被后端拒绝。
...
- 设备创建在事务内锁定型号行并递增下一序号；并发请求不能生成重复设备编号。
...
- Unit: 仅机器编码定位、手机号后四位独立验证、可选订单号消歧、自动会话、序列号
  ...
  附件策略、幂等提交和审核，以及设备大类/型号目录、组合校验、编号前缀校验和
  ERP 已知型号分类。
...
- E2E: 固定入口四字段单次提交、自动创建/恢复登记、数据库落库、管理端查看和
  审核到 Return Delivery；管理端设备大类筛选和新增设备联动下拉。
