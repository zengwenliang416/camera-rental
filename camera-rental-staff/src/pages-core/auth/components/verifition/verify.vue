<template>
  <view v-show="showBox" :class="mode === 'pop' ? 'mask' : ''">
    <view :class="mode === 'pop' ? 'verifybox' : ''" :style="{ 'max-width': `${parseInt(imgSize.width) + 20}px` }">
      ...
      <view :style="{ padding: mode === 'pop' ? '10px' : '0' }" class="verifybox-bottom">
        <VerifyPoints
          v-if="componentType === 'VerifyPoints'" ref="instance" :arith="arith" :bar-size="barSize"
          :block-size="blockSize" :captcha-type="captchaType" :explain="explain" :figure="figure" :img-size="imgSize"
          :mode="mode" :type="verifyType" :p-space="pSpace" @success="success"
        />
        <VerifySlide
          v-if="componentType === 'VerifySlide'" ref="instance" :arith="arith" :bar-size="barSize"
          :block-size="blockSize" :captcha-type="captchaType" :explain="explain" :figure="figure" :img-size="imgSize"
          :mode="mode" :type="verifyType" :p-space="pSpace" @success="success"
        />
      </view>
...
// const { t } = useI18n()
// const { captchaType, mode } = props
const captchaType = computed(() => props.captchaType)
...
function success(captchaValue) {
  closeBox()
  emit('success', captchaValue)
}
defineExpose({ show })
</script>
