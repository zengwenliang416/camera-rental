...
`tests/specnav/rental-configuration-scenarios.cjs`. The existing responsive,
locale, state-feedback, reload, and result aggregation checks for CASE-010 are
unchanged, so the scenario's UI observations remain separate from verification
record emission.

## Component Cohesion / Coupling

The assertion id and expected text remain local to CASE-010. The change uses
the existing `assertion.equal` interface and does not couple the theme and
locale scenario to the runner, Verification Kernel internals, or unrelated
cases.

## Test Quality

The assertion now binds `expected` to the exact CASE-010 contract text instead
...
responsive, locale, or state checks continue to produce a failing equality
assertion. The external Iconify access-policy block reported by the worker
aggregate is outside this assertion-contract repair and is not a code-quality
blocker.

## Reuse / Duplication

The repair reuses the existing equality assertion API. The CASE-010 contract
