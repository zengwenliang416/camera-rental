...
- The repair is not executable by the managed Verification Kernel. The Kernel
  serializes only the `return-review-and-security` scenario function and
  recompiles it in an isolated VM, but the function calls the module-scoped
  `installReturnRegistrationMock` at
  `tests/specnav/customer-return-registration.js:261`. Recompiling the
...
- Make `return-review-and-security` self-contained under Kernel serialization,
