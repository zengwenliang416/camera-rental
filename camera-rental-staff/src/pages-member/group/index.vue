<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['member:group:create'])"
...
const list = ref<MemberGroup[]>([]) // 列表数据
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getMemberGroupPage({
      ...queryParams.value,
      pageNo,
      pageSize,
    })
...
function handleAdd() {
  uni.navigateTo({
    url: '/pages-member/group/form/index',
  })
}

/** 查看详情 */
function handleDetail(item: MemberGroup) {
  uni.navigateTo({
