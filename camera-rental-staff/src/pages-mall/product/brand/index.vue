<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['product:brand:create'])"
...
const list = ref<ProductBrand[]>([]) // 列表数据
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    const data = await getProductBrandPage({ ...queryParams.value, pageNo, pageSize })
...
/** 新增品牌 */
function handleAdd() {
  uni.navigateTo({ url: '/pages-mall/product/brand/form/index' })
}

/** 查看详情 */
function handleDetail(item: ProductBrand) {
  uni.navigateTo({ url: `/pages-mall/product/brand/detail/index?id=${item.id}` })
}

/** 初始化 */
onMounted(() => {
  uni.$on('mall:product-brand:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('mall:product-brand:reload', reload)
})
</script>
