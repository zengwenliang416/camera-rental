# Verification Plan: schedule-center-local-stability-v1

## Verification Scope

- Active change: `schedule-center-local-stability-v1`
- Development handoff: `openspec/changes/schedule-center-local-stability-v1/development/handoff-to-verify.md`
- Runtime: local admin, schedule center, Java backend, MySQL, and Redis.
- Safety boundary: read-only XianGuanJia synchronization is allowed; real
  shipment and all other third-party writes remain disabled.
- Migration boundary: verify migrations 029, 030, and 031 against the local
  backup and migrated database.

## Required Domains

1. Facticity
2. Static
3. Unit
4. Redteam
5. E2E
6. Sensory

## Evidence Plan

- Direct repository evidence beats summaries.
- Every green verdict requires command, file, runtime, screenshot, trace, or
  review evidence.
- Missing evidence is a blocker, not a warning.
- `verify/runtime-evidence.json` must record runtime and browser execution
  evidence. If `development/migrations/manifest.json` has `required=true`, it
  must also record database evidence.
- Browser evidence is stored under
  `runs/schedule-center-verification-20260730/`.
- Database evidence must exclude AppSecret, customer phone, address, and raw
  payload contents.
- A scheduled synchronization may be partially successful when an external
  shop authorization or remote API fails. Verification must distinguish local
  data integrity from third-party availability.

## User-Aligned Test Case Gate

- Generate `verify/user-test-cases.md` and `verify/user-test-cases.json` from
  requirements, acceptance, prototype handoff, development tasks, and handoff.
- Ask the user to approve, edit, remove, or add cases.
- Freeze approval in `verify/user-test-case-signoff.json`.
- Map every approved case across all six domains in
  `verify/domain-case-matrix.json`.
- Six-domain verification is blocked until the signoff status is `approved`.

## Runtime Evidence Gate

- Start the application or relevant service and record the command, output,
  logs, or health check under the `runtime` surface.
- Run the approved user test cases in a real browser or equivalent automation
  and record screenshots, traces, or transcripts under the `browser` surface.
- When migrations are required, run database verification queries and record
  them under the `database` surface.
- A green domain report without matching runtime evidence is invalid.

## Required Commands

- Full rental-module Maven tests and backend package build.
- Admin TypeScript check and production build.
- Schedule-center tests, TypeScript no-emit, and production build.
- Legacy configuration banned-pattern scan.
- Production TS/TSX/CSS 600-line ceiling scan.
- Migration audit-copy comparison and checksum verification.
- Local database schema, switch, tenant, and orphan-relation checks.
- Responsive browser checks at 1440 and 390 CSS pixels.
- Final `git diff --check`.
